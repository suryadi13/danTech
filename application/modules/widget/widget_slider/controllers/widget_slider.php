...
class Widget_slider extends MX_Controller {
	...
	public function index($id_widget,$id_wrapper,$opsi){
		...
		$data['idd']=$id_wrapper;
		$data['daftar'] = $this->m_slider->getwidget($id_widget,$id_wrapper,$opsi[2]->nilai);
	    $d = array ('-','/','\\',',','.','#',':',';','\'','"','[',']','{','}',')','(','|','`','~','!','@','%','$','^','&','*','=','?','+',' ');
		foreach ($data['daftar'] as $key=>$val) {
			...
			$ssdd = $this->m_slider->cekimage($val->id_konten)->result();
			if(empty($ssdd)){
