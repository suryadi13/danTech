...
class Formadmin extends MX_Controller {
	...
	public function tambah()	{
			...
			$rbk = $this->m_hl_slider->getkategori_by_komponen("artikel");
			$data['pilisi']="";
	...
	public function edit()	{
			...
			$data['banyak_post']="<select id='nilai[2]' name='nilai[2]'  class='form-control'>".$tpl_3."</select>";
			$tpl_4= Modules::run("cmshome/durasi_options",@$data['nilai'][2]);
			$data['durasi']="<select id='nilai[3]' name='nilai[3]'  class='form-control'>".$tpl_4."</select>";
...
///////////////////////////////////////////////////////////////////////////////////
	public function formfitur($id_widget,$id_kategories,$opsi){
		...
		$data['daftar'] = $this->m_hl_slider->getwidget($id_widget,$id_kategories,$opsi[2]->nilai);
		foreach($data['daftar'] as $it=>$val){
			@$data['daftar'][$it]->hari = $hh[$val->hari];

			$cek2 = $this->m_hl_slider->sliderkonten($val->id_konten);
			if(!empty($cek2)){
	...
	public function form_upload(){
		...
		$cek2 = $this->m_hl_slider->sliderkonten($_POST['idd']);
		if(!empty($cek2)){
			$data['dcek'] = "ada";
...
/////////////////////////////SLIDER HANDLING //////////////////////
	function saveslider(){
		...
		if($qry->keterangan_appe=="jpg" || $qry->keterangan_appe=="gif" || $qry->keterangan_appe=="png" || $qry->keterangan_appe=="JPG" || $qry->keterangan_appe=="GIF" || $qry->keterangan_appe=="PNG"){
			$sqlstr="DELETE FROM konten_appe WHERE id_konten='".$_POST['id_konten']."' AND tipe='hl_slider'";//// Untuk mencatat jumlah pemakaian file ini
			$this->db->query($sqlstr);
			$sqA = "INSERT INTO konten_appe (id_konten,tipe,foto,foto_from) VALUES ('".$_POST['id_konten']."','hl_slider','".$_POST['path']."','".$_POST['idd']."')";
	...
	function hapusslider(){
		$sq = "SELECT * FROM konten_appe WHERE id_konten='".$_POST['id_konten']."' AND tipe='hl_slider'";
