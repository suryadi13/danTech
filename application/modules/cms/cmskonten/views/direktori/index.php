<div class="row">
	<div class="col-lg-12">
		...
		<div class="btn btn-warning btn-xs pull-right content_post" style="margin-top:10px;" onclick="ppost('direktori/formtambah',0);"><i class="fa fa-plus fa-fw"></i> Tambah Post Item</div>
		</h3>
	</div><!-- /.col-lg-12 -->
</div><!-- /.row -->
								...
								<div class="col-lg-8" style="padding-top:8px;">
									<div class="dropdown"><button class="btn btn-primary dropdown-toggle btn-xs" type="button" id="ddMenuT" data-toggle="dropdown"><span class="fa fa-book fa-fw"></span></button>
										<ul class="dropdown-menu" role="menu" aria-labelledby="ddMenuT">
											...
											?>
											<li role="presentation" <?=($val->komponen=="direktori")?'class="active"':'';?>><a href="<?=site_url('admin/module/cmskonten/'.$val->komponen);?>" role="menuitem" tabindex="-1" style="cursor:pointer;"><i class="fa fa-<?=($val->komponen=="direktori")?"check":"sign-out";?> fa-fw"></i> <?=$val->label;?></a></li>
											<?php
											}
											?>
										</ul>
...
var id_kat = $('#id_kat').val();
	...
	//tombol aksi-->
					table = table+ "<td valign=top style='padding:3px 0px 0px 0px;'>";
						...
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="ppost(\'direktori/formedit\',\'isi\','+item.id_konten+');"><i class="fa fa-edit fa-fw"></i> Edit Data</a></li>';
						if(item.hapus=="ya"){
						table = table+ '<li role="presentation" class="divider">';
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="ppost(\'direktori/formhapus\',\'isi\','+item.id_konten+');"><i class="fa fa-trash fa-fw"></i> Hapus Data</a></li>';
						}
						table = table+ "</ul>";
						table = table+ "</div>";
					table = table+ "</td>";
...
function ppost(tujuan,tipe,id_konten){
	...
	var tab = tab + '<input type="hidden" name="asal" value="direktori">';	
	...
	if(id_kat!=null){	$('#sb_act').html(tab).submit();	}
}
