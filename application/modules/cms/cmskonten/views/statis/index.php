<div class="row">
	<div class="col-lg-12">
		...
		<div class="btn btn-warning btn-xs pull-right content_post" style="margin-top:10px;" onclick="ppost('statis/formedit',0);"><i class="fa fa-plus fa-fw"></i> Tambah Post Item</div>
		</h3></div><!-- /.col-lg-12 -->
</div><!-- /.row -->
								...
								<div class="col-lg-12">
									<div class="dropdown"><button class="btn btn-primary dropdown-toggle btn-xs" type="button" id="ddMenuT" data-toggle="dropdown"><span class="fa fa-book fa-fw"></span></button>
										<ul class="dropdown-menu" role="menu" aria-labelledby="ddMenuT">
											...
											?>
											<li role="presentation" <?=($val->komponen=="statis")?'class="active"':'';?>><a href="<?=site_url('admin/module/cmskonten/'.$val->komponen);?>" role="menuitem" tabindex="-1" style="cursor:pointer;"><i class="fa fa-<?=($val->komponen=="statis")?"check":"sign-out";?> fa-fw"></i> <?=$val->label;?></a></li>
											<?php
											}
											?>
										</ul>
...
var id_kat = $('#id_kat').val();
	$.ajax({
		...
		data:{"hal": hal, "batas": batas,"cari":cari,"komponen":"statis","id_kat":id_kat},
		beforeSend:function(){	
	...
	//tombol aksi-->
					table = table+ "<td valign=top style='padding:3px 0px 0px 0px;'>";
						...
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="ppost(\'statis/formedit\',\'text\','+item.id_konten+');"><i class="fa fa-edit fa-fw"></i> Edit Data</a></li>';
						if(item.hapus=="ya"){
						table = table+ '<li role="presentation" class="divider">';
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="ppost(\'statis/formhapus\',\'text\','+item.id_konten+');"><i class="fa fa-trash fa-fw"></i> Hapus Data</a></li>';
						}
						table = table+ "</ul>";
						table = table+ "</div>";
					table = table+ "</td>";
	//tombol aksi<--
					table = table+ "<td style='padding:3px;'><div><h4 style=\"margin-top:0px;\">"+item.judul+"</h4></div></td>";
					...
					table = table+ '<td style="padding:3px;text-align:center;"><div style="width:120px;"><div class="thumbnail"><div class="caption" style="text-align:center;"><p>';
					table = table+ '<a href="#" class="label label-info" onclick="buka(\'artikel/lampiran_formappe\',\'lampiran\',\''+item.id_konten+'\');"><i class="fa fa-upload fa-fw"></i> Atur File</a></p></div>'+item.lampiran+'</div></div></td>';
					table = table+ "</tr>";
					no++;
				}); //endeach
					$('#list').html(table);
...
function ppost(aksi,tipe,id_konten){
	...
	var tab = tab + '<input type="hidden" name="asal" value="statis">';	
