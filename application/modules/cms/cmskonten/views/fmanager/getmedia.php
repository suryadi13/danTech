...
var mulai=0;
		...
		if((data.isi.length)>0){
			...
			$.each( data.isi, function(index, item){
				...
				if(item.banyak_file==0){	var banyak = item.judul_appe;	} else {	var banyak = item.judul_appe+' <div class="btn btn-warning btn-xs" onClick="loadForm(\'formmedia\',\''+item.idchild +'\','+(parseInt(lvl)+1)+',\''+item.idchild+'\');">: '+item.banyak_file+'</div>';	}
