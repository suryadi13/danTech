...
<div class="row content_post">
	<div class="col-lg-6">
		<div class="panel panel-warning">
			<div class="panel-heading"><i class="fa fa-tags"></i> <b>POST</b></div>
			<div class="panel-body">
											<div class="row" style="line-height:35px;">
												<div class="col-lg-3"><b>Komponen :</b></div>
												<div class="col-lg-9">
														<select id="komponen" name="komponen" class="form-control" onchange="pkomponen();">
															<?php
															foreach($komponen AS $key=>$val){
															?>
															<option value="<?=$val->komponen;?>" <?=($val->komponen=="pengumuman")?"selected":"";?>><?=$val->label;?></option>
															<?php
															}
															?>
														</select>
											...
											<div class="row" style="line-height:35px;">
												<div class="col-lg-3"><b>Kategori :</b></div>
												<div class="col-lg-9">
								...
								<div class="col-lg-12">
									<div class="dropdown"><button class="btn btn-primary dropdown-toggle btn-xs" type="button" id="ddMenuT" data-toggle="dropdown"><span class="fa fa-book fa-fw"></span></button>
										<ul class="dropdown-menu" role="menu" aria-labelledby="ddMenuT">
											<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="ppost('pengumuman/formedit',0);"><i class="fa fa-plus fa-fw"></i> Tambah Post Item</a></li>
										</ul>
										Post Item
									</div>
								</div><!---/.col-lg-12 -->
...
var id_kat = $('#id_kat').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>cmskonten/pengumuman/getdata",
		data:{"hal": hal, "batas": batas,"cari":cari,"komponen":"pengumuman","id_kat":id_kat},
		beforeSend:function(){	
	...
	//tombol aksi-->
					table = table+ "<td valign=top style='padding:3px 0px 0px 0px;'>";
						...
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="ppost(\'pengumuman/formedit\','+item.id_konten+');"><i class="fa fa-edit fa-fw"></i> Edit Data</a></li>';
						table = table+ '<li role="presentation" class="divider">';
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="ppost(\'pengumuman/formhapus\','+item.id_konten+');"><i class="fa fa-trash fa-fw"></i> Hapus Data</a></li>';
						table = table+ "</ul>";
						table = table+ "</div>";
					table = table+ "</td>";
	//tombol aksi<--
					table = table+ "<td style='padding:3px;'>"+item.judul+"</td>";
					table = table+ '<td style="padding:3px;width:150px;text-align:center;">';
					table = table+ '<div style="width:150px;text-align:center;">';
					table = table+ '<div class="thumbnail" style="width:120px;"><div class="caption"><p><a href="#" class="label label-info" onclick="ppost(\'artikel/formupload_foto\','+item.id_konten+');return false;"><i class="fa fa-upload fa-fw"></i> Atur File</a></p></div>'+item.thumb+'</div>';
					table = table+ '</div>';
					table = table+ '</td>';
					...
					table = table+ '<a href="" class="label label-info" onclick="ppost(\'lampiran\',\''+item.id_konten+'\');return false;"><i class="fa fa-upload fa-fw"></i> Atur File</a></p></div>'+item.lampiran+'</div></div></td>';
					table = table+ '<td style="padding:3px;text-align:center;"><div style="width:120px;"><div class="thumbnail"><div class="caption" style="text-align:center;"><p>';
					table = table+ '<a href="" class="label label-info" onclick="viewUppl(\'slider\',\''+item.id_konten+'\');return false;"><i class="fa fa-upload fa-fw"></i> Atur File</a></p></div>'+item.slider+'</div></div></td>';
					table = table+ "</tr>";
					no++;
				}); //endeach
					$('#list').html(table);
				...
				$('#pagingA').html("<input type='hidden' id='inputpaging' value='1'>");
								...
								if(id_kat==""){
									$("#panel_utama").removeClass("panel-success").addClass("panel-warning");
								} else {
									$("#panel_utama").removeClass("panel-warning").addClass("panel-success");
								}
		}, // end success
...
function ppost(aksi,id_konten){
	...
	var tab = tab + '<input type="hidden" name="asal" value="artikel">';	
